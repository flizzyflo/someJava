...
public class Woerterraten {
	...
	private String[] woerter = {"Schifffahrt", "Museum", "Gesellschaft", "Marktplatz", "Pferdescheiße", "Python ist cool"};
	...
	void aktualisiereLoesung(char c) 
	{
		...
		//Iteration über das gesamte zu ratenden Wortes, um bei mehrfachem Auftauchen von "c" alle entsprechenden Sterne in "erratenesWort" zu ersetzen.
		for (int i = 0; i < zuRatendesWort.length(); i++)
		{
			//Wenn das zu ratende Wort an der Stelle "i" die Eingabe "c" enthält, dann wird Eingabe zu True evaluiert und der * wird durch "c" ersetzt.
			if (zuRatendesWort.charAt(i) == c)
			{
				//ersetzen des Sterns an Indexstelle "i" mit Eingabebuchstaben "c".
				erratenesWort.replace(i, i + 1, String.valueOf(c));
